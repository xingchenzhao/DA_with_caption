...
class coco(imdb):
    def __init__(self, image_set, year):
        ...
        self._caption = COCO(self._get_cap_file())
        self._vocab = self._get_vocab_file()
        self._caption_ids = list(self._caption.anns.keys())
        self._img_ids = self._caption.getImgIds()
    ...
    def _get_cap_file(self):
        prefix = 'captions' if self._image_set.find('test') == -1 \
          else 'image_info'
        return osp.join(self._data_path, 'annotations',
                        prefix + '_' + self._image_set + self._year + '.json')

    def _get_vocab_file(self):
        vocab_dir = osp.join(self._data_path, 'annotations',
                             'vocab' + '_' + 'train' + self._year + '.pkl')
    ...
    def caption_at(self, i):
        return self._caption.anns[self._image_index[i]]['caption']
    ...
    # def _load_caption(self, index):  #TODO: 暂时只用一个caption
    #     max_len = 80
    #     annId = self._caption.getAnnIds(imgIds=index, iscrowd=None)
    ...
    #     caption.extend([self._vocab(token) for token in tokens])
    #     caption.append(self._vocab('<end>'))
    #     caplen = len(caption) - 1
    #     for _ in range(max_len - caplen):
    ...
    def _load_caption(self, index):
        ...
        for cap in captions:
            ...
            caption.append(self._vocab('<end>'))
            caplen = len(caption) - 1
            for _ in range(max_len - caplen - 1):
                caption.append(self._vocab('<pad>'))
    ...
    """
        ...
        caption, caplen = self._load_caption(index)
        valid_objs = []
        for obj in objs:
    ...
    def append_flipped_images(self):
        ...
        for i in range(num_images):
            ...
            entry = {
                ...
                'caption': self.roidb[i]['caption'],
                'caplen': self.roidb[i]['caplen']
            }
