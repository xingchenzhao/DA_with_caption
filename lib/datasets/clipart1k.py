...
class clipart1k(imdb):
    def __init__(self, image_set, devkit_path=None):
        imdb.__init__(self, 'clipart1k' + '_' + image_set)
        self._year = '2007'
        self._image_set = image_set
        self._devkit_path = self._get_default_path() if devkit_path is None \
            else devkit_path
        self._data_path = os.path.join(cfg.DATA_DIR, 'clipart')
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        imagesetfile = os.path.join(self._data_path, 'ImageSets', 'Main',
                                    self._image_set + '.txt')
        ...
        if not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        for i, cls in enumerate(self._true_classes):
            if cls == '__background__':
                continue
            filename = self._get_voc_results_file_template().format(cls)
