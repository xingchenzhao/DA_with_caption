...
if __name__ == '__main__':
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            dann_suppression = (2.0 / (1. + np.exp(-args.decay_schedule * p)) -
                                1)
            ...
            if step % args.disp_interval == 0:
                ...
                print("[session %d][epoch %2d][iter %4d/%4d] loss: %.4f, dann_loss: %.4f, lr: %.2e" \
                                        % (args.session, epoch, step, iters_per_epoch, loss_temp, dann_loss_temp, lr))
                ...
                if args.wandb is not None:
                    wandb.log({
                        'fr_loss': loss_temp,
                        'fr_loss_rpn_cls': loss_rpn_cls,
                        'fr_loss_rpn_box': loss_rpn_box,
                        'fr_loss_rcnn_cls': loss_rcnn_cls,
                        'fr_loss_rcnn_box': loss_rcnn_box,
                        'fr_loss_dann': dann_loss_temp,
                        'dann_suppression': dann_suppression
                    })
